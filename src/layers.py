...
class AffineCoupling(nn.Module):
    ...
    Args:
        ...
        split_size: split size of input var : X = [X[0:d] , X[d:D]], split_size=d
    ...
    def __init__(self, input_size, split_size, hidden_size, n_hidden, cond_label_size=None):
        ...
        net = [nn.Linear(split_size + (cond_label_size if cond_label_size is not None else 0),
                         hidden_size + split_size + (cond_label_size if cond_label_size is not None else 0)),
               nn.ReLU(),
               nn.Linear(hidden_size + split_size + (cond_label_size if cond_label_size is not None else 0),
                         hidden_size)]
        for _ in range(n_hidden):
            net += [nn.ReLU(), nn.Linear(hidden_size, hidden_size)]
        net += [nn.ReLU(), nn.Linear(hidden_size, 2 * (
                    input_size - split_size))]  # output_size=2*d, half for s, half for t, s and t have the same dimension of X[d:D]
    ...
    def forward(self, x, y=None):
        ...
        log_abs_det_jacobian = torch.log(
            1 + self.alpha * torch.tanh(s))  # there is another log function than paper, so it is log det
    ...
    def forward(self, x):
        ...
        if self.input_size == n_dim:
            if self.input_size > self.n_cut:  # len(x) > n_cut
                ...
                self.x = None
        elif n_dim <= self.n_cut:  # len(x) < n_cut, inpt_size 可是是原始X的特征维度
...
class W_LU(nn.Module):
    ...
    def forward(self, x, cond_y=None):
        ...
        # upper-triangular matrix, including diagonal elements
        U = torch.triu(LU)
...
class flow_mapping(nn.Module):
    ...
    def __init__(self, input_size, n_depth, n_split_at, n_hidden=2, n_width=32, flow_coupling=0, n_bins=32, r=1.2,
                 bound=50.0):
        super().__init__()
        ...
        for i in range(self.n_depth):
            ...
            self.affine_layers.append(AffineCoupling(input_size,
                                                     ...
                                                     n_hidden=self.n_hidden))  # did not use cond_label_size
    ...
    def forward(self, x):
        ...
        for i in range(self.n_depth):
            ...
            log_det = log_det + tmp_log_det
            z = torch.flip(z, (-1,))  # 翻转列方向的维度，对应论文里的affine
...
class KRnet(nn.Module):
    def __init__(self, base_dist, input_size, n_step, n_depth, width, n_hidden, n_bins=32, r=1.2, bound=50.0,
                 shrink_rate=1.0, rotation=True, device='cpu') -> None:
        super().__init__()
