...
allsamplesy = np.loadtxt('perms-testset.txt',skiprows=0)
nsamples=len(allsamplesy)
allxvec=np.zeros((nsamples, ncell))
for i in range(nsamples):
    yvec = allsamplesy[i]
    xvec = 2.0 ** yvec * 0.1e-15
    allxvec[i]=xvec

# prior***********
starttime=time.time()

datavec=np.zeros((nsamples, nt))
for i in range(nsamples):
    print('simulating sample ', i)
    datavec[i]=singlephase_unsteady_imp(allxvec[i])

endtime=time.time()
print(endtime-starttime)
f = open('qwt-testset.txt','w')
for i in range(nsamples):
    for j in range(0, nt, 2):
        f.write("%e " % (datavec[i, j]))
    f.write("\n")

print("output to vtk")
f = open('result_qmc1sample-test.vtk','w')
...
for i in range(0, len(celllist)):
    f.write("%0.3f\n" % (allxvec[0,i]*1e15))
f.write("SCALARS Permeability2_mD double\n")
f.write("LOOKUP_TABLE default\n")
for i in range(0, len(celllist)):
    f.write("%0.3f\n" % (allxvec[1,i]*1e15))
f.write("SCALARS Permeability3_mD double\n")
f.write("LOOKUP_TABLE default\n")
for i in range(0, len(celllist)):
    f.write("%0.3f\n" % (allxvec[2,i]*1e15))
